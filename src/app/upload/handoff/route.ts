...
export async function POST(request: Request) {
  ...
  const projectJson = String(formData.get("projectJson") ?? "");
  if (!projectJson || new TextEncoder().encode(projectJson).byteLength > MAX_PROJECT_BYTES) {
    return errorPage("The project is empty or larger than the 2 MB community limit.");
  }

  try {
    if (!isStudioProject(JSON.parse(projectJson) as unknown)) {
      return errorPage("This does not look like a complete Design Studio project.");
    }
  } catch {
    return errorPage("The transferred project is not valid JSON.");
  }

  const ledCount = Number(formData.get("ledCount"));
  const draft = {
    projectName: String(formData.get("projectName") ?? "Untitled Pattern").slice(0, 80),
    fileName: String(formData.get("fileName") ?? "studio-pattern.fastled-project.json").slice(0, 160),
    projectJson,
  ...
  return htmlPage(`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Receiving Studio project</title><style>html{color-scheme:dark}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#080a0e;color:#e8edf5;font:15px ui-monospace,monospace}.signal{display:flex;align-items:center;gap:9px;color:#3de2ff}.signal i{width:7px;height:7px;border-radius:50%;background:currentColor;box-shadow:0 0 12px currentColor;animation:p .8s ease-in-out infinite alternate}.signal i:nth-child(2){color:#4438ff;animation-delay:.12s}.signal i:nth-child(3){color:#f037e8;animation-delay:.24s}@keyframes p{to{opacity:.25;transform:translateX(4px)}}</style><div class="signal">Receiving project <i></i><i></i><i></i></div><script>try{sessionStorage.setItem(${storageKey},${scriptValue});location.replace('/upload?from=studio')}catch(e){document.body.textContent='The project arrived, but this browser blocked temporary storage. Please use the file upload instead.'}</script></html>`);
}
